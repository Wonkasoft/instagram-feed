		...
		foreach ( $results['posts'] as $key => $result ) {
			...
			if ( ! empty( $result ) ) {
				...
				if ( ! empty( $result ) ) {
					...
					$preview = ! empty( $image ) ? '<img src="' . wp_get_attachment_image_src( $image_id, 'wonkasoft_instafeed_size' )[0] . '" srcset="' . wp_get_attachment_image_srcset( $image_id, 'wonkasoft_instafeed_size' ) . '" />' : 'N/A';
	...
	public function get_insta_tag_template( $atts ) {
		...
		<?php endif; ?>
				...
				<div id="wonkasoft-instafeed-feed" class="instagram instagram-feeds <?php echo $this->view; ?>" data-view="<?php echo $this->view; ?>">
					...
					if ( ! empty( $results ) && ( ! is_home() || ! is_front_page() ) ) :
